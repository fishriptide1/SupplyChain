...
contract SupplyChain is 
  ...
  modifier verifyCaller (address _address) {
    require(msg.sender == _address, "Not Sender"); 
    _;
  }

  // Define a modifier that checks if the paid amount is sufficient to cover the price
  modifier paidEnough(uint256 _price) { 
    require(msg.value >= _price, "Not enough to meet Price"); 
    _;
  }
  ...
  modifier checkValue(uint256 _upc) {
    ...
    address payable transferTarget = payable(items[_upc].consumerID);
    transferTarget.transfer(amountToReturn);
  ...
  function kill() public {
    if (msg.sender == accountOwner) {
      address payable payAccountOwner = payable (accountOwner);
      selfdestruct(payable(payAccountOwner));
    }
  }
  ...
  public {
    ...
    items[_upc]=Item({ 
      ...
      ownerID                : accountOwner,  // Metamask-Ethereum address of the current owner as the product moves through 8 stages
  ...
  function shipItem(uint256 _upc) public onlyDistributor
    // Call modifier to check if upc has passed previous supply chain stage
    sold(_upc)
